...
namespace FE.Identity.Services
{
    public class RolesServices : IRolesServices
    {
        public void Delete(Roles t)
        {
            try
            {
                using (var cl = new HttpClient())
                {
                    ...
                    HttpResponseMessage res = cl.DeleteAsync("api/Roles/" + t.IdRol.ToString()).Result;
        ...
        {
            List<Models.Roles> aux = new List<Models.Roles>();
            using (var cl = new HttpClient())
            {
                cl.BaseAddress = new Uri(Program.baseurl);
                cl.DefaultRequestHeaders.Clear();
                ...
                if (res.IsSuccessStatusCode)
                {
                    ...
                    aux = JsonConvert.DeserializeObject<List<Models.Roles>>(auxres);
                }
            }
            return aux;
        }
        ...
        {
            Models.Roles aux = new Models.Roles();
            using (var cl = new HttpClient())
            {
                cl.BaseAddress = new Uri(Program.baseurl);
                cl.DefaultRequestHeaders.Clear();
                ...
                {
                    var auxres = res.Content.ReadAsStringAsync().Result;
                    aux = JsonConvert.DeserializeObject<Models.Roles>(auxres);
                }
            }
            return aux;
        }

        public Task<Roles> GetOneByIdAsync(int id)
        {
            throw new System.NotImplementedException();
        }

        public void Insert(Roles t)
        {
            try
            {
                using (var cl = new HttpClient())
        ...
        public void Update(Roles t)
        {
            try
            {
                using (var cl = new HttpClient())
                {
                    ...
                    var byteContent = new ByteArrayContent(buffer);
                    byteContent.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue("application/json");
                    var postTask = cl.PutAsync("api/Roles/" + t.IdRol, byteContent).Result;
